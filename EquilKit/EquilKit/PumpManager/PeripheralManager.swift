...
class PeripheralManager: NSObject {
    private let log = EquilLogger(category: "PeripheralManager")
    private let queue = DispatchQueue(label: "org.nightscout.EquilKit.message-queue")

    private let connectedDevice: CBPeripheral
    private let bluetoothManager: BluetoothManager
    private let pumpManager: EquilPumpManager
    private var completion: ((EquilConnectError?) -> Void)?
    ...
    private var writeQueue: AsyncStream<EquilWriteResult<Any>>.Continuation?
    ...
    public init(
        ...
        _ completion: @escaping (EquilConnectError?) -> Void
    ) {
    ...
    func writePacket(_ packet: any MedtrumBasePacketProtocol) async -> EquilWriteResult<Any> {
        ...
        let stream = AsyncStream<EquilWriteResult<Any>> { continuation in
...
extension PeripheralManager: CBPeripheralDelegate {
    ...
    private func handleHeartbeat(data: Data) {
        ...
        guard Date.now.timeIntervalSince(pumpManager.state.lastSync) > .minutes(5) else {
