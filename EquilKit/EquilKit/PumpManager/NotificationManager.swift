...
class NotificationManager {
    private static let logger = EquilLogger(category: "NotificationManager")
    ...
    private enum Identifiers: String {
        ...
        case batteryLowNotification = "com.nightscout.equilkit.patch-battery-low"
        case batteryEmptyNotification = "com.nightscout.equilkit.patch-battery-empty"
    }
    ...
    public static func batteryLowNotification(_ percent: Double) {
        ensureCanSendNotification {
            let content = UNMutableNotificationContent()
            content.title = String(
                format: String(localized: "Battery low (%d%%)", comment: "Title low battery notification"),
                Int(percent.rounded())
            )
            content.body = String(
                localized: "Your patch battery is running low. Replace the patch soon.",
                comment: "Body low battery notification"
            )

            addRequest(identifier: .batteryLowNotification, content: content, triggerAfter: nil, deleteOld: true)
        }
    }

    public static func batteryEmptyNotification() {
        ensureCanSendNotification {
            let content = UNMutableNotificationContent()
            content.title = String(localized: "Replace your patch now!", comment: "Title replace patch notification")
            content.body = String(
                localized: "Your patch battery is empty!",
                comment: "Body battery empty notification"
            )

            addRequest(identifier: .batteryEmptyNotification, content: content, triggerAfter: nil, deleteOld: true)
        }
    }
