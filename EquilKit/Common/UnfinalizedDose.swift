...
public class UnfinalizedDose {
    ...
    private let logger = EquilLogger(category: "UnfinalizedDose")
    ...
    private func roundBasalRate(_ rate: Double) -> Double {
        EquilPumpManager.onboardingSupportedBasalRates.last(where: { $0 <= rate }) ?? 0
    }
    ...
    public static func defaultBasalDose(basalSchedule: BasalSchedule, insulineType: InsulinType?) -> UnfinalizedDose {
        ...
        let currentRate = basalSchedule.entries.last(where: { Double($0.startTimeSeconds) < nowTimeInterval })?.rate ?? 0
