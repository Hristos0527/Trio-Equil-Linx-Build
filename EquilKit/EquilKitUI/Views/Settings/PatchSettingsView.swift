...
struct PatchSettingsView: View {
    ...
    @State var isEditingLowBattery = false
                ...
                Section {
                    ...
                    .onTapGesture {
                        withAnimation {
                            ...
                            self.isEditingLowBattery = false
                        }
                    }
                    ...
                    sectionItem(
                        title: String(
                            localized: "Notification for low battery",
                            comment: "Label for low battery notification"
                        ),
                        isEditing: isEditingLowBattery,
                        value: $viewModel.lowBatteryNotification,
                        valueRange: [0] + Array(1 ... 8).map {
                            EquilPumpState.batteryPercent(fromPatchVoltage: 2.0 + Double($0) / 10)
                        },
                        formatter: { value in
                            if value == 0 {
                                return String(localized: "Disabled", comment: "label for disabled")
                    ...
                    .onTapGesture {
                        withAnimation {
                            ...
                            self.isEditingNotificationAfterActivation = false
                            self.isEditingLowReservoir = false
                            self.isEditingLowBattery.toggle()
                        }
                    }
                }
